...
/// used as an associated storage by a different PackedUserOperation.
...
contract TestFakeWalletToken is IAccount {
    ...
    public override returns (uint256 validationData) {
        ...
        if (userOp.callData.length == 20) {
            // the first PackedUserOperation sets the second sender's "associated" balance to 0
            address senderToDrain = address(bytes20(userOp.callData[:20]));
            balances[senderToDrain] = 0;
        } else {
            // the second PackedUserOperation will hit this only if included in a bundle with the first one
